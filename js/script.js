...
const trashInterval = setInterval(createTrash, 6000);
if (timeGame.innerText >= "Time: 0:45") {
...
const createBeachGoer = () => {
  ...
  if (
    findEmptySpot(beachGoers, trashArray) &&
    findEmptySpot(beachGoers, beachGoerArray)
  ) {
...
const ditectTrashHit = () => {
  for (let i = 0; i < trashArray.length; i++) {
    if (
      galleria.x < trashArray[i].x + trashArray[i].width &&
...
const ditectBeachGoerHit = () => {
  for (let i = 0; i < beachGoerArray.length; i++) {
    if (
      galleria.x < beachGoerArray[i].x + beachGoerArray[i].width &&
